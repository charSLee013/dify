...
MODEL_PROVIDER_CLASS = CustomProvider
VALIDATE_CREDENTIAL = {
...
def test_is_credentials_valid_or_raise_valid(mocker):
    mocker.patch('core.helper.encrypter.encrypt_token',
                 side_effect=encrypt_side_effect)
    mocker.patch('core.helper.encrypter.decrypt_token',
                 side_effect=decrypt_side_effect)

    mocker.patch('core.model_providers.models.embedding.custom_embeding.CustomEmbeddings.embed_query',
                 return_value="abc")
    mocker.patch('core.model_providers.models.llm.custom_model.CustomChatLLM.generate',
                 return_value="abc")
    ...
    credentials = VALIDATE_CREDENTIAL.copy()
    credentials['completion_type'] = 'chat_completion'
    MODEL_PROVIDER_CLASS.is_model_credentials_valid_or_raise(
        model_name='username/test_model_name',
...
def test_get_model_credentials_custom(mock_decrypt, mocker):
    ...
    encrypted_credential['server_url'] = 'encrypted_' + \
        encrypted_credential['server_url']
    ...
    assert result['server_url'] == 'http://custom-server.com/'
    assert result['headers'] == {'Content-Type': 'application/json'}
    assert result['params'] == {'param1': 'value1'}
